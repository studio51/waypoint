...
module Waypoint
  ...
  class Audit
    ...
    def initialize(window: nil, network: nil, operation: nil, status: nil)
      ...
      @status    = status.presence_in(Waypoint::Run.statuses.keys)
    end
    ...
    #
    def runs
      scope = Waypoint::Run.where(started_at: since..).recent
    ...
    #
    def statuses
      @statuses ||= Waypoint::Run.statuses.keys.index_with(0).merge(
        base.group(:status).count.transform_keys { |value| Waypoint::Run.statuses.key(value) || value.to_s },
      )
    end
    ...
    #
    def incomplete_runs
      @incomplete_runs ||= base.merge(Waypoint::Run.incomplete).count
    end
    ...
    # (see Waypoint::Run::EXPECTED_SQL) — otherwise a routine Xbox refresh that finds
    # 1000 and deliberately takes 3 would read as a 99.7% failure.
    #
    # @return [Float, nil] 0.0-1.0, or nil when nothing ran.
    #
    def completeness
      totals = base.pick(Arel.sql("SUM(synced), SUM(#{ Waypoint::Run::EXPECTED_SQL })"))
    ...
    # @return [Array<String>]
    #
    def networks = @networks ||= Waypoint::Run.where(started_at: since..).distinct.where.not(network: nil).pluck(:network).sort

    # @return [Array<String>]
    #
    def operations = @operations ||= Waypoint::Run.where(started_at: since..).distinct.pluck(:operation).compact.sort
  ...
  private
    ...
    #
    def base
      scope = Waypoint::Run.where(started_at: since..)
      scope = scope.where(network: network)     if network
      scope = scope.where(operation: operation) if operation
