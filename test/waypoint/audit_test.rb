...
#
class Waypoint::AuditTest < ActiveSupport::TestCase
  ...
  test "only counts runs inside the window" do
    ...
    assert_equal 1, Waypoint::Audit.new(window: "1h").total_runs
    assert_equal 2, Waypoint::Audit.new(window: "7d").total_runs
  end

  test "falls back to a sane window for a bad one" do
    assert_equal "24h", Waypoint::Audit.new(window: "nonsense").window
  end
  ...
  test "counts runs whose work was accepted and never reported back" do
    ...
    assert_equal 1, Waypoint::Audit.new.incomplete_runs
  end
  ...
  test "groups faults by who has to act" do
    ...
    parties = Waypoint::Audit.new.faults_by_party
  ...
  test "orders fault kinds by how often they happen" do
    ...
    assert_equal [ "network_unavailable", "our_bug" ], Waypoint::Audit.new.faults_by_kind.keys
    assert_equal 4, Waypoint::Audit.new.total_faults
  end
  ...
  test "ignores faults from runs outside the window" do
    ...
    assert_equal 0, Waypoint::Audit.new(window: "1h").total_faults
    assert_equal 1, Waypoint::Audit.new(window: "30d").total_faults
  end
  ...
  test "filters by network and operation" do
    ...
    assert_equal 1, Waypoint::Audit.new(network: "xbox").runs.count
    assert_equal 1, Waypoint::Audit.new(operation: "trophies").runs.count
    assert_equal 0, Waypoint::Audit.new(network: "xbox", operation: "trophies").runs.count
  end
  ...
  test "filters the run list by status but not the headline" do
    ...
    audit = Waypoint::Audit.new(status: "failed")
  ...
  test "ignores a status that is not a real one" do
    assert_nil Waypoint::Audit.new(status: "banana").status
  end
