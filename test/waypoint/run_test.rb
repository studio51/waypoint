...
class WaypointTest < ActiveSupport::TestCase
  ...
  test "records a non-StandardError exception too" do
    ...
    assert_predicate Waypoint::Run.last, :failed?
  end
  ...
  test "absorbs item outcomes reported after the run returned" do
    ...
    waypoint.absorb(:failed, ProviderError.new(503, "gateway"))
  ...
  test "counters are atomic so concurrent workers do not lose increments" do
    ...
    5.times { Waypoint::Run.find(waypoint.id).absorb(:synced) }
  ...
  test "reaps a run whose items never reported back" do
    waypoint = record { |run| run.found!(10) and run.enqueued!(10) }
    waypoint.update_columns(started_at: (Waypoint.config.stall_after + 1.hour).ago)

    assert_equal 1, Waypoint::Run.reap!
  ...
  test "leaves a recently opened run alone" do
    ...
    assert_equal 0, Waypoint::Run.reap!
  ...
  test "incomplete finds settled runs that lost work" do
    ...
    lost.update_columns(started_at: (Waypoint.config.stall_after + 1.hour).ago)
    Waypoint::Run.reap!

    fine = record { |run| run.found!(1) and run.synced! }

    assert_includes Waypoint::Run.incomplete, lost.reload
    assert_not_includes Waypoint::Run.incomplete, fine.reload
  end
  ...
  test "prunes old successes sooner than old failures" do
    ...
    assert_not Waypoint::Run.exists?(old_ok.id), "a month-old success is noise"
    assert Waypoint::Run.exists?(old_failed.id), "failures are kept for the pattern"
  end
  ...
  test "pruning a run takes its faults with it" do
    ...
    assert_difference("Waypoint::Fault.count", -1) { Waypoint::Run.prune! }
  end
