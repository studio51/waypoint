...
#
# The value of the whole audit rests here: one upstream error can mean the
...
class Waypoint::ClassifierTest < ActiveSupport::TestCase
  ...
  # A host maps its own vocabulary; the gem's job is to consult it. The dummy
  # application configures four codes (see `test/dummy/config/environments/test.rb`).
  #
  test "the host's configured vocabulary is consulted" do
    assert_equal :credentials_expired, Classifier.for_record_error(nil, :authenticator_expired_or_invalid)
    assert_equal :our_data_missing,    Classifier.for_record_error(nil, :internal_missing_data)
  end

  test "a code the host has not mapped falls back rather than raising" do
    assert_equal :our_bug, Classifier.for_record_error(nil, :something_this_app_never_heard_of)
  end
  ...
  test "everything the gem calls a network problem is retryable, except a rejection" do
    [ Timeout::Error, Errno::ECONNRESET, SocketError ].each do |klass|
      assert Classifier.retryable?(Classifier.classify(klass.new("down"))), klass.name
    end

    assert_not Classifier.retryable?(:our_bug)
    assert_not Classifier.retryable?(:credentials_expired)
  end
  ...
  test "any 5xx is the store being broken" do
    [ 500, 502, 503, 504 ].each do |status|
      assert_equal :network_unavailable, Classifier.classify(ProviderError.new(status, "bad")), status.to_s
    end
  end
  ...
  test "both classification routes agree on every mapped status" do
    (Classifier::STATUSES.keys + [ 500, 503 ]).each do |status|
      assert_equal Classifier.classify(ProviderError.new(status, "x")),
  ...
  test "429 is rate limiting, and worth retrying" do
    fault = Classifier.classify(ProviderError.new(429, "slow down"))
  ...
  test "401 is our credentials, not the member's" do
    fault = Classifier.classify(ProviderError.new(401, "nope"))
  ...
  test "404 means the account is gone upstream" do
    assert_equal :account_unreachable, Classifier.classify(ProviderError.new(404, "no such gamer"))
  end
  ...
  test "label still holds the item that failed" do
    ...
    assert_equal I18n.t("waypoint.faults.our_bug"), record.description
  end
end
