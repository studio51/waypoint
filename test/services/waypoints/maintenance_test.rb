...
#
class Waypoints::MaintenanceTest < ActiveSupport::TestCase
  ...
  test "settles runs whose items never reported back" do
    abandoned = waypoint!(status: :running, started_at: (Waypoint::STALL_AFTER + 1.hour).ago, found: 10, enqueued: 10)

    result = Waypoints::Maintenance.new.perform
  ...
  test "prunes past retention" do
    ...
    assert_not Waypoint.exists?(old_ok.id)
  end
  ...
  test "a run reaped long ago is prunable" do
    ...
    Waypoints::Maintenance.new.perform

    assert_not Waypoint.exists?(stale.id), "a four-month-old abandoned run should not still be here"
  end
  ...
  test "leaves healthy recent runs alone" do
    ...
    assert Waypoint.exists?(recent_ok.id)
  end
end
